...
function jsonLdTypeIncludes(value, required) {
    if (typeof value === 'string')
        return value === required;
    if (Array.isArray(value))
        return value.some((entry) => entry === required);
    return false;
}
function assertSignatureV02(value) {
    if (!isRecord(value))
...
export function canonicalizeManifestPayload(manifest) {
    const unsigned = {};
...
export function create(input) {
    ...
    if (input.shards !== undefined)
        manifest.shards = input.shards;
    if (input.claims !== undefined)
        manifest.claims = input.claims;
    if (input.consents !== undefined)
        manifest.consents = input.consents;
    if (input.devices !== undefined)
        manifest.devices = input.devices;
    if (input.pointers !== undefined)
        manifest.pointers = input.pointers;
    if (input.extra) {
...
export function assertValidManifest(value, options = {}) {
    ...
    if (value.shards !== undefined) {
        ...
        for (const shard of value.shards) {
            if (!isRecord(shard))
                throw new Error('shard must be an object');
